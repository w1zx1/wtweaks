...
Full (`wtweaks_0.1.0.apbx`) — 52 tasks in apply order. Lite (`wtweaks_0.1.0-lite.apbx`) — 25 tasks (QoL subset + Explorer restart).
...
| Remove Internet Explorer (`remove-ie.yml`) | Disables the `Internet-Explorer-Optional-amd64` feature, removing its useless Start menu shortcut | ✅ | ❌ |
...
- Full: 52 tasks (all of the above, in `Configuration/main.yml` order).
